...
def inside_date_range(iso_str: Optional[str], start: Optional[datetime], end: Optional[datetime]) -> bool:
    if not (start or end):
        return True
    if not iso_str:
        return False
...
def init_driver(headless: bool, use_uc: bool, fast_mode: bool):
    if use_uc:
        try:
            ...
            print("⚠ Không thể dùng undetected-chromedriver:", e)
    opts = Options()
    if headless:
...
def smart_scroll(driver, outer_rounds=2, step_px=1100):
    ...
    for _ in range(scroll_rounds):
        ...
        for p in raw:
            ...
            if key and key not in collected:
                ...
                # Stop-by-time: nếu có start_dt và thấy post cũ hơn start_dt, dừng sau vòng này
                if start_dt and iso:
                    try:
                ...
                if inside_date_range(iso, start_dt, end_dt):
                    collected[key] = {
        ...
        if n >= max_posts or stable_rounds >= 3 or stop_by_time_hit:
            break
        smart_scroll(driver, outer_rounds=2, step_px=1100)
...
class ScrapeThread(threading.Thread):
    ...
    def run(self):
        ...
        try:
            ...
            if not (app.var_use_cookies.get() and cookie_path.exists()):
            ...
            for idx in range(self.start_index, len(urls)):
                ...
                # ghi checkpoint trước khi bắt đầu URL này
                write_checkpoint(
                    out_dir=Path(app.var_outdir.get().strip() or "."),
                    urls=urls,
                    next_index=idx,  # nếu stop giữa chừng, resume sẽ chạy lại URL này
                    filters={
                ...
                rows, hit_time_stop = scrape_url(
                    driver=driver,
                    url=url,
                    wait_sec=int(app.var_wait.get()),
                    scroll_rounds=int(app.var_rounds.get()),
                    max_posts=int(app.var_max.get()),
                    start_dt=start_dt,
                    end_dt=end_dt,
                    apply_hashtag_fix=app.var_fix_hashtag.get(),
                ...
                files = save_outputs(rows, Path(app.var_outdir.get().strip() or "."), url)
                app.log(f"  → Đã lưu: {files['csv']}")

                # update checkpoint: đã xong URL này → tiếp theo là idx+1
                write_checkpoint(
                    out_dir=Path(app.var_outdir.get().strip() or "."),
                    urls=urls,
                    next_index=idx + 1,
            ...
            # Hoàn tất: xoá checkpoint nếu đã chạy hết
            next_idx = read_checkpoint(Path(app.var_outdir.get().strip() or ".")) or {}
            if next_idx and next_idx.get("next_index", 0) >= len(self.urls):
                remove_checkpoint(Path(app.var_outdir.get().strip() or "."))

            # mở thư mục, phân tích nhanh
...
class App(tk.Tk):
    def __init__(self):
        ...
        # State vars (ẩn khỏi UI)
        self.var_headless = tk.BooleanVar(value=False)
        self.var_use_uc = tk.BooleanVar(value=False)
        self.var_fastmode = tk.BooleanVar(value=False)
        ...
        # Khi mở app: nếu thấy checkpoint → gợi ý resume (không bắt buộc)
    ...
    def _build_ui(self):
        ...
        ttk.Checkbutton(mode_frame, text="Batch mode (nhiều URL)", variable=self.var_mode_batch, command=self._toggle_mode).pack(anchor="w")
        ...
        ttk.Checkbutton(pfrm, text="Preview 5 before save", variable=self.var_preview).grid(row=0, column=6, padx=8)

        # Buttons
        bfrm = ttk.Frame(frm)
        bfrm.grid(row=4, column=0, columnspan=5, sticky="we", pady=(8,4))
        ...
        # Progress + ETA
        p2 = ttk.Frame(frm)
        p2.grid(row=5, column=0, columnspan=5, sticky="we")
        ...
        self.txt.grid(row=6, column=0, columnspan=5, sticky="nsew", pady=(6,0))
    ...
    def init_driver(self):
        return init_driver(
            ...
            fast_mode=self.var_fastmode.get(),
        )
