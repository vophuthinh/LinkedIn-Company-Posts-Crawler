# -*- coding: utf-8 -*-
"""
LinkedIn Company Posts — PRO (Upgraded, UI Enhanced)
------------------------------------------------------------------
NOTE: Logic giữ nguyên so với bản gốc, chỉ chỉnh UI (giao diện) cho gọn - đẹp - dễ dùng.
- Thêm style hiện đại (theme 'clam'), font thống nhất, padding, separator.
- Nút có icon Unicode (▶, ■, 🔑, 💾, 📂) và group theo từng khu vực (LabelFrame).
- Log chuyển sang ScrolledText có thanh cuộn, font monospace.
- Progressbar dày hơn, có ETA rõ ràng.
- Tooltips "bay" giúp giải thích tùy chọn (hover để xem).
- Phím tắt: F5 (Start), Ctrl+S (Save cookies), Ctrl+O (Mở thư mục xuất), Esc (Stop).
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("LinkedIn Company Posts")
    ...
    def _build_ui(self):
        ...
        ttk.Label(hdr, text='LinkedIn Company Posts — PRO', style='Header.TLabel').pack(anchor='w')
        ttk.Label(hdr, text='Crawler UI gọn - đẹp · Giữ nguyên logic gốc · Có tooltip & phím tắt',
        ...
        opts.pack(fill='x')
        ttk.Checkbutton(opts, text='Preview 5 before save', variable=self.var_preview).pack(side='left', padx=(0, 14))
        # ttk.Checkbutton(opts, text='Use cookies (auto load/save)', variable=self.var_use_cookies).pack(side='left', padx=(0, 14))
        # ttk.Checkbutton(opts, text='Use Undetected-CD', variable=self.var_use_uc).pack(side='left', padx=(0, 14))
        ttk.Checkbutton(opts, text='Headless', variable=self.var_headless).pack(side='left', padx=(0, 14))
        ttk.Checkbutton(opts, text='Dark Mode', variable=self.var_dark_mode, command=self._toggle_theme).pack(
            side='left')
